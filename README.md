...
1. Set the variables inside this repository's `extensions/homeassistant/config.sh`.
   - If the image endpoint requires HTTP Basic Auth, set `BASIC_AUTH_USERNAME` and `BASIC_AUTH_PASSWORD`. Leave both empty to disable authentication. Percent-encode URL-reserved characters in the username or password. Because the Kindle fetches over plain HTTP, these credentials are sent without TLS protection.
